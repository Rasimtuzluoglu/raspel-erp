...
mvn -B test -q             # Run 1015 tests (H2 in-memory)
...
npm run test                # Run 678 tests (Vitest) + coverage gate
npm run lint                # ESLint
...
| Tests | JUnit 5 (1015) + Vitest (678) + Cypress (9 E2E spec) |
...
- Backend: 1015 tests (JUnit 5, H2, Mockito) + JaCoCo coverage gate, must pass before commit
- Frontend: 678 tests (Vitest) + coverage gate, zero ESLint warnings required; `npm run i18n:check` clean
