...
mvn -B test -q             # Run 977 tests (H2 in-memory)
...
npm run test                # Run 670 tests (Vitest) + coverage gate
npm run lint                # ESLint
...
| Tests | JUnit 5 (977) + Vitest (670) + Cypress (9 E2E spec) |
...
- Backend: 977 tests (JUnit 5, H2, Mockito) + JaCoCo coverage gate, must pass before commit
- Frontend: 670 tests (Vitest) + coverage gate, zero ESLint warnings required; `npm run i18n:check` clean
