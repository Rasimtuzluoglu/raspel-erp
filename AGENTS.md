...
mvn -B test -q             # Run 1033 tests (H2 in-memory)
...
npm run test                # Run 692 tests (Vitest) + coverage gate
npm run lint                # ESLint
...
| Tests | JUnit 5 (1033) + Vitest (692) + Cypress (9 E2E spec) |
...
│   ├── views/                # 72 views (lazy-loaded)
│   ├── components/            # 51 shared components
...
- Backend: 1033 tests (JUnit 5, H2, Mockito) + JaCoCo coverage gate, must pass before commit
- Frontend: 692 tests (Vitest) + coverage gate, zero ESLint warnings required; `npm run i18n:check` clean
