<template>
  <Dialog :visible="visible" @update:visible="$emit('update:visible', $event)"
    header="TC Kimlik Dogrulayici" :modal="false" :style="{ width: '320px' }" :draggable="false">
    <div class="tc-form">
      <div class="form-satir">
        <label>TC Kimlik No</label>
        <InputText v-model="tc" placeholder="12345678901" maxlength="11" class="w-full" @input="kontrol" />
      </div>
      <div v-if="sonuc !== null" class="tc-sonuc" :class="sonuc ? 'gecerli' : 'gecersiz'">
        <i :class="sonuc ? 'pi pi-check-circle' : 'pi pi-times-circle'" />
        <strong>{{ sonuc ? 'Gecerli TC Kimlik No' : 'Gecersiz TC Kimlik No' }}</strong>
      </div>
      <p class="tc-uyari">* Algoritmik dogrulama yapilir. Resmi dogrulama icin NVI servisleri kullanilmalidir.</p>
    </div>
  </Dialog>
</template>
...
const kontrol = () => {
  ...
  if (val.length !== 11 || val[0] === '0') { sonuc.value = false; return }
...
.tc-sonuc { display: flex; align-items: center; gap: 10px; padding: 14px; border-radius: 8px; font-size: 14px; }
.tc-sonuc i { font-size: 24px; }
.gecerli { background: var(--green-50); color: var(--green-700); }
.gecersiz { background: var(--red-50); color: var(--red-700); }
.tc-uyari { font-size: 11px; color: var(--text-muted); margin: 0; }
</style>
