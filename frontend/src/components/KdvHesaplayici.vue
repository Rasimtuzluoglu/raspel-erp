  ...
  <Dialog :visible="visible" @update:visible="$emit('update:visible', $event)"
    ...
    <div class="kdv-form">
      <div class="form-satir">
        <label>Tutar</label>
        <InputNumber v-model="tutar" :min="0" class="w-full" />
      </div>
      <div class="form-satir">
        <label>KDV Orani (%)</label>
        <Select v-model="oran" :options="oranlar" class="w-full" />
      </div>
      <div class="kdv-sonuc">
        <div class="sonuc-satir"><span>KDV Tutari:</span><strong>{{ format(kdvTutari) }}</strong></div>
        <div class="sonuc-satir"><span>KDV Dahil Toplam:</span><strong>{{ format(kdvDahil) }}</strong></div>
        <div class="sonuc-satir"><span>KDV Haric:</span><strong>{{ format(tutar || 0) }}</strong></div>
      </div>
    </div>
  </Dialog>
</template>
...
<style scoped>
.kdv-form { display: flex; flex-direction: column; gap: 12px; }
.form-satir label { display: block; font-size: 13px; color: var(--text-secondary); margin-bottom: 4px; }
.kdv-sonuc { background: var(--surface-ground); border-radius: 8px; padding: 16px; }
