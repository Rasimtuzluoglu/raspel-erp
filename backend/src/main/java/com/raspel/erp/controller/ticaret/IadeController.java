...
public class IadeController {
    ...
    @PostMapping
    public ResponseEntity<IadeDTO> olustur(@RequestBody IadeDTO dto, HttpServletRequest request) {
        Long sirketId = (Long) request.getAttribute("sirketId");
        return ResponseEntity.status(HttpStatus.CREATED).body(iadeService.olustur(dto, sirketId));
    }

    @PutMapping("/{id}")
    public ResponseEntity<IadeDTO> guncelle(@PathVariable Long id, @RequestBody IadeDTO dto) {
